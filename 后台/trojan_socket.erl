...
% 定义默认配置参数
% 🎀 我们的小小配置盒子～
% 这里存放着所有重要的默认设置，就像一个精心准备的首饰盒
% 每个配置都是一颗闪闪发光的小宝石呢～
-define(DEFAULT_NAME, "木马后台").    % 🏰 我们小屋的名字
-define(DEFAULT_PORT, 8888).         % 🚪 迎接客人的门牌号
-define(DEFAULT_IP, {0,0,0,0}).      % 🌏 在哪里等待客人呢
-define(DEFAULT_BUFSIZE, 1024).      % 📦 准备多大的收纳盒
...
% start/0 函数现在变得简洁啦，它负责开启监听，然后把招待客人的任务交给 accept_loop
% 启动服务器
% 💝 这是我们最重要的启动函数哦～
% 它就像一个温柔的管家，负责：
% 🎀 先打印漂亮的欢迎横幅
